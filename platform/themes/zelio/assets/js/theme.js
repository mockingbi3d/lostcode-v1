    ...
    const initCardScroll = () => {
        ...
        Promise.all(imagePromises).then(() => {
            requestAnimationFrame(() => {
                ...
                const maxCardHeight = Math.max(...cardInners.map(inner =>
                    inner ? inner.offsetHeight : 0
                ))
                ...
                cardsContainer.style.setProperty('--card-height', `${maxCardHeight}px`)
                ...
                Array.from(cards).forEach((card, index) => {
                    ...
                    const cardHeight = cardInner.offsetHeight + offsetTop

                    ScrollObserver.Element(nextCard, {
                        offsetTop,
                        offsetBottom: window.innerHeight - cardHeight,
